    ...
    read: (ctx) => {
        ...
        for (const field of ctx.args) {
            ...
            while (remaining >= 8 && ((position & 7) === 0)) {
                const byteIndex = ctx.offset + (position >> 3);
                value = (value << 8n) | BigInt(ctx.view.getUint8(byteIndex));
                position += 8;
                remaining -= 8;
            }
            ...
            while (remaining > 0) {
                ...
                const bit = (ctx.view.getUint8(byteIndex) >> bitIndex) & 1;
        ...
        ctx.offset += Math.ceil(bitSize / 8);
    ...
    write: (ctx, struct) => {
        ...
        for (const field of ctx.args) {
            ...
            while (remaining >= 8 && ((bitPos & 7) === 0)) {
                const byteIndex = ctx.offset + (bitPos >> 3);
                const byte = Number((fieldVal >> BigInt(remaining - 8)) & 0xFFn);
                ctx.view.setUint8(byteIndex, byte);
                bitPos += 8;
                remaining -= 8;
            }
            ...
            for (let i = remaining - 1; i >= 0; i--) {
                ...
                const byteIndex = ctx.offset + (globalBit >> 3);
                const bitIndex = 7 - (globalBit & 7);

                const old = ctx.view.getUint8(byteIndex);
                ctx.view.setUint8(byteIndex, (old & ~(1 << bitIndex)) | (bit << bitIndex));
            }
